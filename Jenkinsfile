pipeline {
  ...
  stages {
     ...
     stage('Fortify Build') {
      steps {
        sh 'sudo sourceanalyzer -v -b 123456 src/*'
        sh 'sudo sourceanalyzer -v -b 123456 -scan -f results.fpr'
        sh 'sudo cp /var/lib/jenkins/workspace/SecurityShepherd_master@2/results.fpr /var/lib/jenkins/workspace/deploy/results.fpr  '
      }
    }
    stage('Fortify - SAST') {
      agent {
        docker {
          ...
          args '-v /var/lib/jenkins/workspace/SecurityShepherd_master@2:/fortify -e KONDUKTO_HOST=${konduktoAddress} -e KONDUKTO_TOKEN=U1U2dlA4SmhFN1BaTFc3ZkRhVVVBTzNEakNtQlBNV3cweHlsaDB2Z284N0ROOURxRE9iUmJ1WlFRT0Jk'
        }

      }
      steps {
        sh 'kdt scan -p SecurityShepard -t fortify -b master -f /fortify/results.fpr'
      }
    }
