    ...
    try {
      ...
      if (csrfToken.isEmpty()) {
        ...
        if (responseBody.contains("Error Occurred")) {
          ...
          if (ModulePlan.isTournamentFloor()) {
            String message =
                "Tournament Mode was enabled after what should have been a failed request";
            log.fatal(message);
            fail(message);
          } else if (!ModulePlan.isIncrementalFloor()) {
            String message =
                "ModulePlan no longer returning possitive for CTF Floor Check after what should"
                    + " have been a failed request";
            log.fatal(message);
            fail(message);
          }
        } else {
