...
public class XssLessonIT {
  ...
  public String getModuleDoPost(String searchTerm, String csrfToken, int expectedResponseCode)
      throws Exception {
    try {
    ...
    try {
      ...
      if (servletResponse.contains("You must be getting funky")) {
        ...
        fail(message);
          ...
          "No session was detected with your request. Are you signed in?")) {
        String message = new String("Did not get 'Unauthenticated' Response");
        log.fatal(message);
        fail(message);
      } else if (servletResponse.contains(
          "Results")) // This string is in all possible responses except unauthenticated
      {
