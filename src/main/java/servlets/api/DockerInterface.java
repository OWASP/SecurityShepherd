    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ((Validate.validateSession(ses)) && (ses.getAttribute("userRole").equals("admin"))) {
            ...
            if (Validate.validateTokens(tokenCookie, tokenParmeter)) {
                ...
                if (dockerID != null) {
                    response.setHeader("Content-Type", "application/json");
                    log.info("POST request for docker information received for " + dockerID + " by " + ses.getAttribute("userName").toString());
                    String action = request.getParameter("action");
                    if (action.equalsIgnoreCase("start")) {
                        docker.startContainer(dockerID);
                        out.print(docker.getContainerData(dockerID));
                    } else if (action.equalsIgnoreCase("stop")) {
                        docker.stopContainer(dockerID);
                        out.print(docker.getContainerData(dockerID));
                    } else if (action.equalsIgnoreCase("restart")) {
                        docker.restartContainer(dockerID);
                        out.print(docker.getContainerData(dockerID));
                    }
                }
            }
