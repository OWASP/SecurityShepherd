 ...
 *
 */
public class SSORegister extends HttpServlet
{
	private static final long serialVersionUID = 1L;
	private static org.apache.log4j.Logger log = Logger.getLogger(SSORegister.class);
	private static String defaultClass = new String();
	/**
	 * Initiated by sso-register.jsp. If successful a player is added to the system, otherwise there is no change.
	...
	*/
	...
	throws ServletException, IOException
	{
		...
		log.debug("**** servlets.SSORegister ***");
		...
		if(OpenRegistration.isEnabled())
		{
			...
			try
			{
				...
				if(paramToken.compareTo(sessToken) == 0)
				{
					...
					notNull = (userName != null && userID != null);
					log.debug("Ensuring strings are not empty");
					notEmpty = (!userName.isEmpty() && !userID.isEmpty());
