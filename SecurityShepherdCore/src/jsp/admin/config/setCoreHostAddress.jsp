...
<%
	ShepherdExposedLogManager.logEvent(request.getRemoteAddr(), request.getHeader("X-Forwarded-For"), "DEBUG: setCoreHostAddress.jsp *************************");
...
{
	ShepherdExposedLogManager.logEvent(request.getRemoteAddr(), request.getHeader("X-Forwarded-For"), "DEBUG(setCoreHostAddress.jsp): tokenCookie Error:" + htmlE.toString());
}
...
%>
	<h1 class="title">Core Server Address</h1>
	Your Host's IP address can be accessed by going to the command prompt of the server and typing; <a>ipconfig</a> for windows servers or <a>ifconfig</a> for linux/mac servers<br/><br/>
	Current Core Server Address = <a><%= encoder.encodeForHTML(ExposedServer.getSecureUrl()) %></a>
	<br/>
	<br/>
